...
function createFormStore(name, initial, devtools) {
  ...
  const withMw = (creator) => process.env.NODE_ENV !== "production" && devtools ? __require("zustand/middleware").devtools(subscribeWithSelector(creator), { name }) : subscribeWithSelector(creator);
  const storeImpl = createStore(withMw(() => base));
  return {
...
function useForm(opts) {
  ...
  const [formState, setFormState] = React.useState(store.getState().formState);
  React.useEffect(() => {
    const unsub = store.subscribe((s) => setFormState(s.formState));
    return () => unsub();
  }, [store]);
  const runResolverAndApply = React.useCallback(async (values) => {
  ...
  const Provider = React.useCallback(({ children }) => {
...
export { createBatcher, createFormStore, getAtPath, parsePath, setAtPath, useForm, useFormStore };
